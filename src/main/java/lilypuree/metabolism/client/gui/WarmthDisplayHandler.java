...
public class WarmthDisplayHandler extends Screen {
    ...
    public void onFoodDraw(RenderGuiOverlayEvent.Pre event) {
        ...
        renderHealth(mc, mc.getWindow().getGuiScaledWidth(), mc.getWindow().getGuiScaledHeight(), event.getGuiGraphics());
        mc.getProfiler().pop();
    }

    public void renderHealth(Minecraft mc, int width, int height, GuiGraphics guiGraphics) {
        ...
        addRightHeight(10 + (warmthRows - 1) * rowHeight);
    ...
    private static void addRightHeight(int amount) {
        getGui().rightHeight += amount;
    }

    private static int getTickCount() {
        return getGui().getGuiTicks();
    }
