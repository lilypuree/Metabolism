...
public abstract class DebugRenderOverlay {
    ...
    public void renderTick(RenderGuiOverlayEvent.Post event) {
        ...
        if (!this.isHidden() && !this.debugText.isEmpty() && !mc.isPaused() && !mc.options.renderDebug && event.getOverlay() == VanillaGuiOverlay.CHAT_PANEL.type()) {
            ...
            if (scale > 0.0F) {
                ...
                for (String line : debugText) {
                    this.drawLine(event.getGuiGraphics(), font, line, x, y, 16777215);
                    y += LINE_HEIGHT;
                }
                matrix.popPose();
            }
